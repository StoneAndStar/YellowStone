...
/datum/antagonist/mercenary/equip()
	...
	L.set_default_language(LANGUAGE_NEOHONGO)

	//Normal mercs can't speak common
	L.remove_language(LANGUAGE_COMMON)

	//And we'll give them a random chinese/shi name to start off with
	var/datum/language/lang = all_languages[LANGUAGE_NEOHONGO]
